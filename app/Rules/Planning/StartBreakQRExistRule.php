...
class StartBreakQRExistRule implements ValidationRule
{
    ...
    public function validate(string $attribute, mixed $value, Closure $fail): void
    {
        ...
        if ($plans->count() > 1) { # check if more than one plan is started
            $fail('Cannot start break, Please stop past plan.');
        } elseif ($plans->count() == 1 && $plans->first()->break_started) { # check if plan has started to take break
            $fail('Break already started.');
        } elseif ($plans->count() == 0) {
            $fail('No plan start break.');
        }
    }
}
