...
class ContractService
{
    ...
    public function generateEmployeeContract($employee_contract_id, $contract_status) 
    {

        try {
            DB::connection('master')->beginTransaction();
                $url  = env('CONTRACTS_URL') . config('contracts.GENERATE_CONTRACT_ENDPOINT');
                $body = $this->getEmployeeContractTemplate($employee_contract_id);
    ...
    public function getEmployeeContractFiles($employee_profile_id, $status, $contract_id = '') # ['signed', 'unsigned', 'approved']
    {
